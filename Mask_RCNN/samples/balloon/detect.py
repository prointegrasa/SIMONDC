...
def run(
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    line = (int(str(cls)[7:-2]), *xywh, float(str(conf)[7:-1])) #if save_conf else (cls, *xywh)  # label format
                    barcodes_coordinates.append(line)
    ...
    return(barcodes_coordinates)


def parse_opt(imgpath:str,weightspath:str):


    parser = argparse.ArgumentParser()

    parser.add_argument('--weights', nargs='+', type=str, default=weightspath, help='model path or triton URL')
    parser.add_argument('--source', type=str, default=imgpath, help='file/dir/URL/glob/screen/0(webcam)')
...
def detect(imgpath:str,weightspath:str):
    opt = parse_opt(imgpath,weightspath)
    coords = main(opt)
    return(coords)
